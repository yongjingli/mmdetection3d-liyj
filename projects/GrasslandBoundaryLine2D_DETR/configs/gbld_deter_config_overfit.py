...
custom_imports = dict(
    imports=['projects.GrasslandBoundaryLine2D.gbld2d',      # 将2D卷积的project也同时导入
             'projects.GrasslandBoundaryLine2D_DETR.gbld2d_detr'], allow_failed_imports=False)

experiment_name = "debug_gbld_detr_dataset"
work_dir = './work_dirs/' + experiment_name

data_root = '/home/liyongjing/Egolee/hdd-data/data/dataset/glass_lane/debug_overfit_gbld_detr'
...
fixed_pts_num_per_line = 20      # 每条线点的固定数量
...
# 模型部分的定义
model = dict(
    type='GBLDDetrMono2Dtector',              # 新建一个glass_edge_line_detector的类
    ...
    bbox_head=dict(
        type='GBLDDetrMono2DHead',
        num_queries=100,
        transform_feat_idx=2,          # 输入的特征fpn不同stage的特征,选择其中一个特征进行transform，-1为最后一个特征,分辨率最小
        select_gt_stages_index=0,       # detr只是对单个stage的特征进行预测,选择对应stage的gt, 设置为gt_instances
        select_gt_stages_maps={'gt_line_instances_stages': "gt_line_instances",
                               'gt_line_labels_stages': 'gt_line_labels',
                               'gt_bboxes_stages': 'bboxes',
                               'gt_labels_stages': 'labels',
                               # 'gt_line_maps_stages': 'gt_line_maps'
                               },     # 设置gt_instances包含哪些内容,并进行相应名称的转换

        encoder=dict(  # DetrTransformerEncoder
            num_layers=2,   # 6 进行过拟合实验时,采用的num_layers越小，越容易实现过拟合
        ...
        decoder=dict(  # DetrTransformerDecoder
            num_layers=3,  # 6   进行过拟合实验时,采用的num_layers越小，越容易实现过拟合
        ...
        bbox_decode=dict(
            # type='mmdet.DETRHead',
            # type='GBLDDETRBboxHead',   # 只有bbox
            type='GBLDDETRLineHead',     # bbox和line
            num_classes=len(classes),
            embed_dims=256,
            num_reg_fcs=2,
            num_pts_fcs=2,
            num_pts_per_vec=fixed_pts_num_per_line,
            ...
            loss_pts=dict(type='mmdet.L1Loss', loss_weight=5.0),
            loss_iou=dict(type='mmdet.GIoULoss', loss_weight=2.0),
        train_cfg=dict(
            assigner=dict(
                type='GBLDDETRHungarianAssigner',
                ...
                pts_cost=dict(type='GBLDDETROrderPtsCost', weight=5.0),  # 自定义的点集cost
                )),
        ...
        seg_decode=dict(
                ...
                with_orient=False,                  # 是否预测点的方向
                with_visible=False,                 # 是否预测点的可见性
                with_hanging=False,                 # 是否预测点的悬空属性
                with_covered=False,                 # 是否预测点的草遮挡属性
                ...
                num_discriminative_emb=128,  # discriminative聚类特征的维度
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        pipeline=[
            ...
            dict(type='GgldDetrLoadLines', name="load_gt_lines"),

            # dict(type='GgldRandomRotate', prob=0.3, angle_range=12),
            # dict(type='GgldRandomRotate', prob=1.0, angle_range=12),      # debug

            # 该方法实现还有问题, TODO
            # dict(type='GgldDetrRandomCrop', prob=0.5, max_margin_scale=0.4, keep_ratio=True),
            # dict(type='GgldRandomCrop', prob=1.0, max_margin_scale=0.4, keep_ratio=True),   # debug

            dict(type='GgldDetrResize', scale=input_size,
            # keep_ratio=False,
            keep_ratio=True,
            ),

            # dict(type='GgldDetrColor', prob=0.5),
            ...
            # 在padding前进行flip，保证padding的都是在右下角
            # dict(type='GgldDetrRandomFlip', prob=0.5, direction=['horizontal']),
            ...
            dict(type='GgldDetrLineCapture',
                 gt_down_scales=[4, 8, 16],   # up_scale = 2: [2, 4, 8] # up_scale = 4: [1, 2, 4]
                 num_classes=num_classes,
                 filter_small_line=True,      # 是否过滤短线
                 filter_length=32,            # 过滤曲线的长度
                 sample_dist=1,
                 num_samples=250,
                 padding=False,
                 fixed_num=fixed_pts_num_per_line,                # 曲线采样的点数
                 padding_value=-10000,
                 ),
            ...
            dict(
                ...
                meta_keys=['gt_lines', 'sample_idx', 'scale_factor',
                           'ori_shape', 'img_shape']
            ),
        ],
        # 标注名称不在列表的将会被过滤
    ...
    dataset=dict(
        ...
        pipeline=[
            ...
            dict(
                ...
                meta_keys=['eval_gt_lines', 'sample_idx', 'scale_factor', 'ori_shape', 'img_shape',
...
# train_cfg = dict(type='EpochBasedTrainLoop', max_epochs=12, val_begin=5, val_interval=1)
# train_cfg = dict(type='EpochBasedTrainLoop', max_epochs=250, val_begin=50, val_interval=20)
# train_cfg = dict(type='EpochBasedTrainLoop', max_epochs=250, val_begin=5, val_interval=1)   # debug
# train_cfg = dict(type='EpochBasedTrainLoop', max_epochs=1000, val_begin=5, val_interval=1)   # debug
train_cfg = dict(type='EpochBasedTrainLoop', max_epochs=10000, val_begin=5, val_interval=1)   # debug
...
param_scheduler = [
    ...
    dict(
        ...
        # milestones=[
        #     20,  # debug
        #     666,  # 大概在[end - begin] * 8/12
        #     916,  # 大概在[end - begin] * 11/12
        # ],

        # 10000
        milestones=[
            200,  # debug
            6660,  # 大概在[end - begin] * 8/12
            9160,  # 大概在[end - begin] * 11/12
        ],
...
default_hooks = dict(
    ...
    checkpoint=dict(type='CheckpointHook', interval=200),    # 设置保存checkpoint的间隔

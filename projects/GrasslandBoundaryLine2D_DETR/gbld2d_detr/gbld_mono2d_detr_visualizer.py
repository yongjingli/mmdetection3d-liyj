...
class GbldDetrVisualizer(DetLocalVisualizer):
    ...
    def __init__(
        ...
        line_color: Optional[Union[str, Tuple[int]]] = None,
        with_text: Union[bool] = True,
    ...
    def draw_gt_lines(self, image, gt_lines, classes, palette):
        classes = self.classes if classes is None else classes

        self.set_image(image)
        labels = [int(gt_line["category_id"]) for gt_line in gt_lines]
        ...
        for gt_line in gt_lines:
            ...
            x_datas = np.array([points[i:i+2, 0] for i in range(len(points)-1)])
            y_datas = np.array([points[i:i+2, 1] for i in range(len(points)-1)])

            # x_datas, y_datas, colors, line_styles, line_widths
            self.draw_lines(x_datas, y_datas, color, line_widths=self.line_width)

        # draw text
        if self.with_text:
            txt_positions = [gt_line["points"][len(gt_line["points"])//2] for gt_line in gt_lines]
            text_color = self.text_color if self.text_color is not None else palette

            for i, (pos, label) in enumerate(zip(txt_positions, labels)):
                class_txt = str(label) if classes is None else classes[label]
                color = text_color[label]
                self.draw_texts(
                    class_txt,
                    pos,
                    colors=color,
                    font_sizes=int(self.font_size),
                    bboxes=[{
                        'facecolor': 'black',
                        'alpha': 0.8,
                        'pad': 0.7,
    ...
    def draw_pred_lines(self, image, pred_lines, classes, palette):
        self.set_image(image)

        classes = self.classes if classes is None else classes

        labels = [int(pred_line[0][4]) for pred_line in pred_lines]
        max_label = int(max(labels) if len(labels) > 0 else 0)
        palette = get_palette(palette, max_label + 1)

        for pred_line in pred_lines:
            category_id = int(pred_line[0][4])
            points = pred_line[:, :2]
        ...
        # draw text
        if self.with_text:
            txt_positions = [pred_line[len(pred_line)//2, :2] for pred_line in pred_lines]
            text_color = self.text_color if self.text_color is not None else palette

            for i, (pos, label) in enumerate(zip(txt_positions, labels)):
                class_txt = str(label) if classes is None else classes[int(label)]
                color = text_color[label]
                self.draw_texts(
                    class_txt,
                    pos,
    ...
    def add_datasample(self,
        ...
        if draw_gt and data_sample is not None:
            if 'ori_eval_gt_lines' in data_sample:
                # 选择其中一个stage的结果
                if len(data_sample.ori_eval_gt_lines) > 0:

                    gt_lines = data_sample.ori_eval_gt_lines
                    assert 'img' in data_input
                    img = data_input['img']
                    if isinstance(data_input['img'], Tensor):
                        img = data_input['img'].permute(1, 2, 0).numpy()
                        img = img[..., [2, 1, 0]]  # bgr to rgb
                    gt_img_data = self.draw_gt_lines(img, gt_lines, classes, palette)

        if draw_pred and data_sample is not None:
            if 'pred_instances' in data_sample:
                # 选择其中一个stage的结果
                if len(data_sample.pred_instances.stages_result[0]) > 0:
                    pred_lines = data_sample.pred_instances.stages_result[0][0]
                    img = data_input['img']
                    if isinstance(data_input['img'], Tensor):
                        img = data_input['img'].permute(1, 2, 0).numpy()
                        img = img[..., [2, 1, 0]]  # bgr to rgb

                    pred_img_data = self.draw_pred_lines(img, pred_lines, classes, palette)

...
class GBLDDetrInstanceLines(object):
    ...
    def start_end_points(self):
        ...
        instance_se_points_tensor[:, 0] = torch.clamp(instance_se_points_tensor[:, 0], min=0, max=self.max_x)
        instance_se_points_tensor[:, 1] = torch.clamp(instance_se_points_tensor[:, 1], min=0, max=self.max_y)
        instance_se_points_tensor[:, 2] = torch.clamp(instance_se_points_tensor[:, 2], min=0, max=self.max_x)
        instance_se_points_tensor[:, 3] = torch.clamp(instance_se_points_tensor[:, 3], min=0, max=self.max_y)
    ...
    def bbox(self):
        ...
        instance_bbox_tensor[:, 0] = torch.clamp(instance_bbox_tensor[:, 0], min=0, max=self.max_x)
        instance_bbox_tensor[:, 1] = torch.clamp(instance_bbox_tensor[:, 1], min=0, max=self.max_y)
        instance_bbox_tensor[:, 2] = torch.clamp(instance_bbox_tensor[:, 2], min=0, max=self.max_x)
        instance_bbox_tensor[:, 3] = torch.clamp(instance_bbox_tensor[:, 3], min=0, max=self.max_y)
    ...
    def fixed_num_sampled_points(self):
        ...
        instance_points_tensor[:, :, 0] = torch.clamp(instance_points_tensor[:, :, 0], min=0, max=self.max_x)
        instance_points_tensor[:, :, 1] = torch.clamp(instance_points_tensor[:, :, 1], min=0, max=self.max_y)
    ...
    def shift_fixed_num_sampled_points_v2(self):
        ...
        for idx, instance in enumerate(self.instance_list):
            ...
            shift_num = pts_num - 1
            final_shift_num = self.fixed_num - 1
            if instance_label == 3 and 0:  # 暂时不清楚id为3的特殊处理 TODO
                # import ipdb;ipdb.set_trace()

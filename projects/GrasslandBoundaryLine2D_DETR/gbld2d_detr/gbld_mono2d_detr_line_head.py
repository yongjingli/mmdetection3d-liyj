...
class GBLDDETRLineHead(BaseModule):
    ...
    def __init__(
            ...
            loss_pts: ConfigType = dict(type='L1Loss', loss_weight=5.0),
        ...
        if class_weight is not None and (self.__class__ is GBLDDETRLineHead):
    ...
    def _init_layers(self) -> None:
        ...
        self.pts_ffn = FFN(
            ...
            dropout=0.0,
            add_residual=False)
        self.fc_pts = Linear(self.embed_dims, self.num_pts_per_vec * 2)
    ...
    def forward(self, hidden_states: Tensor) -> Tuple[Tensor]:
        ...
        layers_pts_preds = self.fc_pts(
            self.activate(self.pts_ffn(hidden_states))).sigmoid()

        hidden_num, batch_size, num_query, dim = layers_pts_preds.shape
        layers_pts_preds = layers_pts_preds.view(hidden_num, batch_size, num_query, self.num_pts_per_vec, 2)
        return layers_cls_scores, layers_bbox_preds, layers_pts_preds
    ...
    def loss_by_feat(
        ...
        losses_cls, losses_bbox, losses_iou, loss_pts = multi_apply(
            self.loss_by_feat_single,
            all_layers_cls_scores,
            all_layers_bbox_preds,
            all_layers_pts_preds,
        ...
        loss_dict['loss_pts'] = loss_pts[-1]
        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i, loss_iou_i,  loss_pts_i in \
                zip(losses_cls[:-1], losses_bbox[:-1], losses_iou[:-1], loss_pts[:-1]):
            loss_dict[f'd{num_dec_layer}.loss_cls'] = loss_cls_i
            loss_dict[f'd{num_dec_layer}.loss_bbox'] = loss_bbox_i
            loss_dict[f'd{num_dec_layer}.loss_iou'] = loss_iou_i
            loss_dict[f'd{num_dec_layer}.loss_iou'] = loss_pts_i
            num_dec_layer += 1
        return loss_dict

    def loss_by_feat_single(self, cls_scores: Tensor, bbox_preds: Tensor, pts_preds: Tensor,
                            batch_gt_instances: InstanceList,
        ...
        factors = []
        factors_pts = []
        for img_meta, bbox_pred, pts_pred in zip(batch_img_metas, bbox_preds, pts_preds):
            ...
            factor_pts = pts_preds.new_tensor([img_w, img_h]).unsqueeze(0).unsqueeze(0).repeat(
                                               pts_pred.size(0), self.num_pts_per_vec, 1)
            factors_pts.append(factor_pts)

        factors = torch.cat(factors, 0)
        factors_pts = torch.cat(factors_pts, 0)
        ...
        # 是否需要在原图尺寸上进行loss的计算
        # pts_preds和pts_targets都是归一化后的坐标，需要re-scale到输入图像大小的尺寸
        # pts_preds = pts_preds * factors_pts
        batch_size, num_pred, fix_num, dim = pts_preds.shape
        pts_preds = pts_preds.reshape(batch_size * num_pred, fix_num, dim)
        # pts_preds = pts_preds * factors_pts
        # pts_targets = pts_targets * factors_pts
        ...
        loss_pts = self.loss_pts(
            pts_preds, pts_targets, pts_weights, avg_factor=num_total_pos)

        return loss_cls, loss_bbox, loss_iou, loss_pts

    def get_targets(self, cls_scores_list: List[Tensor],
                    bbox_preds_list: List[Tensor],
                    pts_preds_list: List[Tensor],
                    batch_gt_instances: InstanceList,
    ...
    def get_gt_shift_points(self, gt_instances):
        gt_shift_points = None
        if self.gt_shift_pts_pattern == 'v0':
            gt_shift_points = gt_instances.gt_line_instances.shift_fixed_num_sampled_points_v2
    ...
    def _get_targets_single(self, cls_score: Tensor, bbox_pred: Tensor,
        ...
        pts_factor = pts_pred.new_tensor([img_w, img_h]).unsqueeze(0).unsqueeze(0)
        pts_pred = pts_pred * pts_factor

        pred_instances = InstanceData(scores=cls_score, bboxes=bbox_pred, pts=pts_pred)
        gt_shift_points = self.get_gt_shift_points(gt_instances)
        gt_instances.gt_shift_points = gt_shift_points.to(bbox_pred.device)
        ...
        if len(gt_bboxes) != 0:
            ...
            # print("====="*10)
            # print("gt_labels[pos_assigned_gt_inds]:", gt_labels[pos_assigned_gt_inds])
            ...
            # pts_targets
            pts_targets = torch.zeros_like(pts_pred, dtype=pts_pred.dtype)
            pts_weights = torch.zeros_like(pts_pred, dtype=pts_pred.dtype)

            assigned_shift = order_index[pos_inds, pos_assigned_gt_inds]  # order_index: num_perd, num_gt
            pos_gt_pots_targets = gt_instances.gt_shift_points[pos_assigned_gt_inds, assigned_shift, :, :]
            factor_pts = pts_pred.new_tensor([img_w, img_h]).unsqueeze(0).unsqueeze(0)
            pos_gt_pots_targets = pos_gt_pots_targets/factor_pts
            pts_targets[pos_inds] = pos_gt_pots_targets

            pts_weights[pos_inds] = 1.0

        else:
            # in case for no gt-boxes, add by liyj-20231222
            # set all background category
            labels = bbox_pred.new_full((num_bboxes, ), self.num_classes, dtype=torch.long)
            # set all weight 1.0
            label_weights = bbox_pred.new_ones(num_bboxes)

            # no target boxes
            bbox_targets = torch.zeros_like(bbox_pred, dtype=bbox_pred.dtype)
            # set all weight 0.0
            bbox_weights = torch.zeros_like(bbox_pred, dtype=bbox_pred.dtype)

            pts_targets = torch.zeros_like(pts_pred, dtype=pts_pred.dtype)
            pts_weights = torch.zeros_like(pts_pred, dtype=pts_pred.dtype)

        return (labels, label_weights, bbox_targets, bbox_weights, pts_targets, pts_weights, pos_inds,
                neg_inds)
    ...
    def predict_by_feat(self,
        ...
        pts_preds = layer_bbox_pts[-1]

        result_list = []
        for img_id in range(len(batch_img_metas)):
            cls_score = cls_scores[img_id]
            bbox_pred = bbox_preds[img_id]
            pts_pred = pts_preds[img_id]
            img_meta = batch_img_metas[img_id]
            results = self._predict_by_feat_single(cls_score, bbox_pred, pts_pred,
                                                   img_meta, rescale)
    ...
    def _predict_by_feat_single(self,
        ...
        # pts
        pts_pred[:, :, 0::2] = pts_pred[:, :, 0::2] * img_shape[1]
        pts_pred[:, :, 1::2] = pts_pred[:, :, 1::2] * img_shape[0]

        pts_pred[:, :, 0::2].clamp_(min=0, max=img_shape[1])
        pts_pred[:, :, 1::2].clamp_(min=0, max=img_shape[0])

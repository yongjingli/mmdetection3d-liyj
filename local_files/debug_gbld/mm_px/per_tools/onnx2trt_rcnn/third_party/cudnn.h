...
typedef struct cudnnSpatialTransformerStruct
...
typedef enum {
  ...
  CUDNN_TENSOR_NCHW_VECT_C = 2, /* each image point is vector of element of C,
                                   vector length in data type */
} cudnnTensorFormat_t;
...
/* Helper function to return the minimum size of the index space to be passed to
 * the reduction given the input and output tensors */
...
/* The NaN propagation enum applies to only the min and max reduce ops; the
 * other reduce ops propagate NaN as usual. */
...
/* Convolution functions: All of the form "output = alpha * Op(inputs) + beta *
 * output" */
...
/* Fused conv/bias/activation operation : y = Act( alpha1 * conv(x) + alpha2 * z
 * + bias ) */
...
/* Softmax functions: All of the form "output = alpha * Op(inputs) + beta *
 * output" */
...
/* Pooling functions: All of the form "output = alpha * Op(inputs) + beta *
 * output" */
...
/* Activation functions: All of the form "output = alpha * Op(inputs) + beta *
 * output" */
cudnnStatus_t CUDNNWINAPI
 ...
 * Any of the provided pointers can be NULL (no corresponding value will be
 * returned)
 */
...
/* LRN cross-channel forward computation. Double parameters cast to tensor data
 * type */
...
/* LRN cross-channel backward computation. Double parameters cast to tensor data
 * type */
...
#define CUDNN_BN_MIN_EPSILON                                                   \
  0.0 /* Minimum epsilon allowed to be used in the Batch Normalization formula \
       */

/*
 * Derives a tensor descriptor from layer data descriptor for BatchNormalization
 * scale, invVariance, bnBias, bnScale tensors. Use this tensor desc for
 * bnScaleBiasMeanVarDesc and bnScaleBiasDiffDesc in Batch Normalization forward
...
typedef enum {
  ...
  CUDNN_BATCHNORM_OPS_BN_ADD_ACTIVATION =
      2, /* do batchNorm, then elemWiseAdd, then activation */
} cudnnBatchNormOps_t;
...
/* Computes y = BN(x). Also accumulates moving averages of mean and inverse
 * variances */
 ...
 * y[i] = bnScale[k]*(x[i]-estimatedMean[k])/sqrt(epsilon+estimatedVariance[k])
 * + bnBias[k] with bnScale, bnBias, runningMean, runningInvVariance tensors
 * indexed according to spatial or per-activation mode. Refer to
 * cudnnBatchNormalizationForwardTraining above for notes on function arguments.
 */
...
/*helper function to determine size of the states to be passed to
 * cudnnSetDropoutDescriptor */
cudnnStatus_t CUDNNWINAPI cudnnDropoutGetStatesSize(cudnnHandle_t handle,
                                                    size_t *sizeInBytes);

/*helper function to determine size of the reserve space to be passed to dropout
...
typedef enum {
  ...
  CUDNN_GRU = 3, /* Using h' = tanh(r * Uh(t-1) + Wx) and h = (1 - z) * h' + z *
                    h(t-1); */
} cudnnRNNMode_t;
...
typedef enum {
  ...
  CUDNN_RNN_SINGLE_INP_BIAS =
      1, /* rnn cell formulas use one input bias in input GEMM */
  CUDNN_RNN_DOUBLE_BIAS =
      2, /* default, rnn cell formulas use two bias vectors */
  CUDNN_RNN_SINGLE_REC_BIAS =
      3 /* rnn cell formulas use one recurrent bias in recurrent GEMM */
} cudnnRNNBiasMode_t;
...
typedef enum {
  CUDNN_RNN_DATA_LAYOUT_SEQ_MAJOR_UNPACKED =
      0, /* padded, outer stride from one time-step to the next */
  CUDNN_RNN_DATA_LAYOUT_SEQ_MAJOR_PACKED =
      1, /* sequence length sorted and packed as in basic RNN api */
  CUDNN_RNN_DATA_LAYOUT_BATCH_MAJOR_UNPACKED =
      2, /* padded, outer stride from one batch to the next */
...
/* mathPrec in the RNN descriptor is determines compute math precision, modified
 * by cudnnMathType_t */
...
typedef enum {
  CUDNN_ATTN_QUERYMAP_ALL_TO_ONE =
      0, /**< multiple Q-s when beam width > 1 map to a single (K,V) set */
  CUDNN_ATTN_QUERYMAP_ONE_TO_ONE = 1, /**< multiple Q-s when beam width > 1 map
                                         to corresponding (K,V) sets */
} cudnnAttnQueryMap_t;
...
typedef enum {
  ...
  CUDNN_WGRAD_MODE_SET =
      1, /* write partial gradients to wgrad output buffers */
} cudnnWgradMode_t;
...
typedef void (*cudnnCallback_t)(cudnnSeverity_t sev, void *udata,
...
typedef enum {
  ...
  /* [per channel scale], [per channel bias], [activation], convolution,
     [generate BN stats] */
  CUDNN_FUSED_SCALE_BIAS_ACTIVATION_CONV_BNSTATS = 0,
  /* [per channel scale], [per channel bias], [activation],
     convolutionBackwardWeights */
  CUDNN_FUSED_SCALE_BIAS_ACTIVATION_WGRAD = 1,
  /* utility for BN training in BN-conv fusion */
  /* computes the equivalent scale and bias from ySum ySqSum and learned scale,
     bias */
  /* optionally update running stats and generate saved stats */
  CUDNN_FUSED_BN_FINALIZE_STATISTICS_TRAINING = 2,
  /* utility for BN inference in BN-conv fusion */
  /* computes the equivalent scale and bias from learned running stats and
     learned scale, bias */
  CUDNN_FUSED_BN_FINALIZE_STATISTICS_INFERENCE = 3,
  /* reserved for future use: convolution, [per channel scale], [per channel
     bias], [residual add], [activation] */
  CUDNN_FUSED_CONV_SCALE_BIAS_ADD_ACTIVATION = 4,
  /* reserved for future use: [per channel scale], [per channel bias], [residual
     add],  activation, bitmask */
...
typedef enum {
  ...
  /* set CUDNN_PARAM_BN_EQSCALEBIAS_DESC: pass previously initialized
     cudnnTensorDescriptor_t */
  /* get CUDNN_PARAM_BN_EQSCALEBIAS_DESC: pass previously created
     cudnnTensorDescriptor_t */
  ...
  /* set ACTIVATION_DESC: pass previously initialized
  ...
  /* set CUDNN_PARAM_BN_SCALEBIAS_MEANVAR_DESC: pass previously initialized
     cudnnTensorDescriptor_t */
  /* get CUDNN_PARAM_BN_SCALEBIAS_MEANVAR_DESC: pass previously created
     cudnnTensorDescriptor_t */
  CUDNN_PARAM_BN_SCALEBIAS_MEANVAR_DESC = 19,
  /* set/get CUDNN_PARAM_BN_SCALE_PLACEHOLDER: pass
     cudnnFusedOpsPointerPlaceHolder_t* */
  CUDNN_PARAM_BN_SCALE_PLACEHOLDER = 20,
  /* set/get CUDNN_PARAM_BN_BIAS_PLACEHOLDER: pass
     cudnnFusedOpsPointerPlaceHolder_t* */
  CUDNN_PARAM_BN_BIAS_PLACEHOLDER = 21,
  /* set/get CUDNN_PARAM_BN_SAVED_MEAN_PLACEHOLDER: pass
     cudnnFusedOpsPointerPlaceHolder_t* */
  CUDNN_PARAM_BN_SAVED_MEAN_PLACEHOLDER = 22,
  /* set/get CUDNN_PARAM_BN_SAVED_INVSTD_PLACEHOLDER: pass
     cudnnFusedOpsPointerPlaceHolder_t* */
  CUDNN_PARAM_BN_SAVED_INVSTD_PLACEHOLDER = 23,
  /* set/get CUDNN_PARAM_BN_RUNNING_MEAN_PLACEHOLDER: pass
     cudnnFusedOpsPointerPlaceHolder_t* */
  CUDNN_PARAM_BN_RUNNING_MEAN_PLACEHOLDER = 24,
  /* set/get CUDNN_PARAM_BN_RUNNING_VAR_PLACEHOLDER: pass
     cudnnFusedOpsPointerPlaceHolder_t* */
  ...
  /* set BN_Z_EQSCALEBIAS_DESC: pass previously initialized
     cudnnTensorDescriptor_t */
  ...
  /* set ACTIVATION_BITMASK_DESC: pass previously initialized
     cudnnTensorDescriptor_t */
  /* get ACTIVATION_BITMASK_DESC: pass previously created
     cudnnTensorDescriptor_t */
  CUDNN_PARAM_ACTIVATION_BITMASK_DESC = 31,
  /* set/get ACTIVATION_BITMASK_PLACEHOLDER: pass
     cudnnFusedOpsPointerPlaceHolder_t* */
  ...
  /* set/get CUDNN_PARAM_BN_DSCALE_PLACEHOLDER: pass
     cudnnFusedOpsPointerPlaceHolder_t* */
  CUDNN_PARAM_BN_DSCALE_PLACEHOLDER = 37,
  /* set/get CUDNN_PARAM_BN_DBIAS_PLACEHOLDER: pass
     cudnnFusedOpsPointerPlaceHolder_t* */
...
/* DEPRECATED routines to be removed next release :
   User should use the non-suffixed version (which has the API and functionality
   of _v6 version) Routines with _v5 suffix has the functionality of the
   non-suffixed routines in the CUDNN V6
 */

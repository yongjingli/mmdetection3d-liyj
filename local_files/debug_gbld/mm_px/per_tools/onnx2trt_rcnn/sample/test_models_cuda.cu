    ...
    if( 0 == thread_type ){
	    for (int nT = 0; nT < MAX_TEST; nT++) {
	      ...
	      for (unsigned int ch = 0; ch < engine_num; ch++) {
	        threads[ch] = xpilot_os::Thread(testRunEngine, ch, batch_sizes[ch], input[ch].inputStream, 0,
	    ...
	    for (unsigned int ch = 0; ch < engine_num; ch++) {
	      threads[ch] =
	          xpilot_os::Thread(testRunEngine1, ch, batch_sizes[ch], input[ch].inputStream,

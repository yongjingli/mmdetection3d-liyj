...
__global__ void res2channle(float *out) {
  const int idx = blockIdx.x / 2;
  out[idx] = idx / 255.0f;
}

__global__ void res2channle2(float *out) {
  const int idx = blockIdx.x >> 1;
  out[idx] = idx * (1/255.0f);
}
  ...
  if (NULL != pCreateEngine && NULL != pRunEngine &&
    ...
    if( 0 == thread_type ){
	    ...
	    for (int nT = 0; nT < MAX_TEST; nT++) {
		    ...
		    } else if( 4 == thread_type ){ // multi-stream in main thread, sync by stream
		        pRunEngine(0, batch_sizes[0], input[0].inputStream, 0, NULL, -1);
		        for (unsigned int ch = 0; ch < engine_num; ch++) {
              if( ch < engine_num-1 )
		            pRunEngine(ch+1, batch_sizes[ch+1], input[ch+1].inputStream, 0, NULL, -1);

              EngineBuffer *bufInfo = bufferInfo[ch];
              int batch = std::min(bufInfo[0].nMaxBatch, batch_sizes[ch]);
              int size = bufInfo[1].nBufferSize / bufInfo[1].nMaxBatch * batch;
              cudaMemcpyAsync( output[ch], bufInfo[1].p, size,
                              cudaMemcpyDeviceToHost, (cudaStream_t)stream[ch]);
		          cudaStreamSynchronize((cudaStream_t)stream[ch]);
		          float ms = std::chrono::duration<float, std::milli>(
		    ...
		    } else if( 5 == thread_type ){ // multi-stream in main thread, sync by stream
		        pRunEngine(0, batch_sizes[0], input[0].inputStream, 0, NULL, -1);
		        for (unsigned int ch = 0; ch < engine_num; ch++) {
              if( ch < engine_num-1 )
		            pRunEngine(ch+1, batch_sizes[ch+1], input[ch+1].inputStream, 0, output[ch+1], -1);

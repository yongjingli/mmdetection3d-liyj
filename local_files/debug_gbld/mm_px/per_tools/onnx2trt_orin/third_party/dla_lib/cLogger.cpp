...
CLogger::CLogger() : m_level(LEVEL_ERR), m_style(LOG_STYLE_NORMAL), m_logFile(stdout) {}
...
void CLogger::SetLogLevel(LogLevel level) { m_level = (level > LEVEL_DBG) ? LEVEL_DBG : level; }
...
void CLogger::LogLevelMessageVa(LogLevel level, const char *functionName, uint32_t lineNumber, const char *format,
                                va_list ap) {
  char str[256] = {
      '\0',
  };

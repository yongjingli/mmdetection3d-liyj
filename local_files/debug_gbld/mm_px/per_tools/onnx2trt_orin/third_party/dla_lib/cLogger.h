...
//! Set log level
#define SET_LOG_LEVEL(level) CLogger::GetInstance().SetLogLevel(level)
...
class CLogger {
 ...
 private:
  ...
  void LogLevelMessageVa(LogLevel eLevel, const char *pszFunctionName, uint32_t sLineNumber, const char *pszFormat,
                         va_list ap);

#ifndef COMMON_JSON_GASON_H_
#define COMMON_JSON_GASON_H_
...
struct JsonIterator {
    ...
    JsonIterator operator[](char *key) const {
      for (JsonIterator i = *this ; i != JsonIterator{nullptr} ; ++i) {
      ...
      std::string error_message = "Out of range execption: There is no value with key ";
      error_message += key;
      throw std::out_of_range(error_message);
    }

    JsonIterator operator[](int index) const {
      if (p->value.getTag() != JSON_ARRAY)
        return JsonIterator{nullptr};
      JsonIterator result = JsonIterator{p->value.toNode()};
      for (int i = 0 ; i < index && result != JsonIterator{nullptr} ; i++)
        ++result;
      if (result != JsonIterator{nullptr})
        return JsonIterator{result->value.toNode()};
      else {
        std::string error_message = "Out of range exception: Accessing index " + std::to_string(index);
        throw std::out_of_range(error_message);
      }
    }
};
...
inline JsonIterator find(JsonValue o, char *key) {
  for (JsonIterator i = begin(o) ; i != end(o) ; ++i) {
    if (i->key == key) {
      return i;
    } else {
      continue;

...
#include "xpilot/services/dds_forwarder/dds_forwarder.h"
...
int main(int argc, char *argv[]) {
  ...
  while (true) {
    if (num_messages <= 0) {
      ...
      model_mode = static_cast<df::ModelMode::inner_enum>((num_messages % 2) + 1);
      env_mode = static_cast<df::EnvMode::inner_enum>((num_messages % 3) + 1);
      sleep_ms = distribution(generator);
    }
    LOG(INFO) << "Switching to ModelMode " << model_mode << ", EnvMode " << env_mode << " in " << sleep_ms << " ms";
    fsd_detection_status_msg.model_mode(model_mode.underlying());
    fsd_detection_status_msg.env_mode(env_mode.underlying());
    int auto_st = 0;
    for (int i = 0; i < sleep_ms / kMsgSentIntervalMs; i++) {
      ...
      std::this_thread::sleep_for(std::chrono::milliseconds(kMsgSentIntervalMs));
      if (kSendSMGSMsg) {
